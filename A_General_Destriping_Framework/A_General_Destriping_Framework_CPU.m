...
function DATA_est = A_General_Destriping_Framework_CPU(DATA_noisy, para)
%% 
[n1, n2, n3] = size(DATA_noisy);

%% Settings and initializing
V = (DATA_noisy); % observed data
...
if strcmp(regularization, 'HTV')
    ...
    Y1{1} = (zeros(size(L{1}(U))));
    
    Gamma1_U    = (1./(precon1_D_v + precon1_D_h + 1));
...
elseif strcmp(regularization, 'SSTV')
    ...
    Y1{1} = (zeros(size(L{1}(U))));
    Y1{2} = (zeros(size(L{2}(U))));
    
    Gamma1_U = (1./(precon1_D_v.*precon1_D_b + precon1_D_h.*precon1_D_b + 1));
...
elseif strcmp(regularization, 'ASSTV')
    ...
    Y1{1} = (zeros(size(L{1}(U))));
    Y1{2} = (zeros(size(L{2}(U))));
    Y1{3} = (zeros(size(L{3}(U))));
    
    Gamma1_U    = (1./(precon1_D_v + precon1_D_h + precon1_D_b + 1));
...
elseif strcmp(regularization, 'SSTV+TNN')
    ...
    proxL{1} = @(Y, Gamma) prox_l1(Y, Gamma);
    proxL{2} = @(Y, Gamma) prox_l1(Y, Gamma);
    proxL{3} = @(Y, Gamma) prox_TNN(Y, 100*Gamma);
    
    Y1{1} = (zeros(size(L{1}(U))));
    Y1{2} = (zeros(size(L{2}(U))));
    Y1{3} = (zeros(size(L{3}(U))));
    
    Gamma1_U    = (1./(precon1_D_v.*precon1_D_b + precon1_D_h.*precon1_D_b + 2));
...
elseif strcmp(regularization, 'l0l1HTV')
    ...
    Gamma1_U    = (1./(precon1_D_v.*precon1_D_b + precon1_D_h.*precon1_D_b + precon1_D_v + precon1_D_h + 1)); 
...
elseif strcmp(regularization, 'ATV+NN')
    ...
    Y1{2} = (zeros(size(L{2}(U))));
    Y1{3} = (zeros(size(L{3}(U))));
    Y1{4} = (zeros(size(L{4}(U))));
    
    Gamma1_U    = (1./(precon1_D_v + precon1_D_h + precon1_D_b + 2)); 
...
while is_converged == 0
    ...
    % 収束レートの計算
    error_U = (sqrt(sum((U - U_next).^2, "all")/sum(U.^2, "all")));
